...
export const getCrossOwnersCollectionsV1Options: RouteOptions = {
  cache: {
    privacy: "public",
  ...
  response: {
    ...
    }).label(`getCrossOwnersCollections${version.toUpperCase()}Response`),
    failAction: (_request, _h, error) => {
      logger.error(
        `get-cross-owners-collections-${version}-handler`,
  ...
  handler: async (request: Request) => {
    ...
    if (query.collections) {
      ...
      for (const collection of query.collections) {
        const collectionsFilter = `'${_.replace(collection, "0x", "\\x")}'`;

        if (_.isUndefined((query as any).collectionsFilter)) {
          (query as any).collectionsFilter = [];
        }

        (query as any).collectionsFilter.push(collectionsFilter);
      }
    ...
    try {
      ...
      logger.error(`get-cross-owners-collections-${version}-handler`, `Handler failure: ${error}`);
      throw error;
    }
  },
};
