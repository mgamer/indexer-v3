...
if (config.doBackgroundWork) {
  const worker = new Worker(
    QUEUE_NAME,
    async (job) => {
      const { fromEventTimestamp, toEventTimestamp } = job.data;
      ...
      const results = await redshift!.manyOrNone(
        `
          ...
          AND event_timestamp < $/toEventTimestamp/ 
      ...
      if (results.length >= limit) {
        ...
        logger.info(
          QUEUE_NAME,
          `Triggering Next Job. fromEventTimestamp=${lastResult.event_timestamp}, toEventTimestamp=${toEventTimestamp}`
        );

        await addToQueue(lastResult.event_timestamp, toEventTimestamp);
      }
    },
...
export const addToQueue = async (fromEventTimestamp: string, toEventTimestamp: string) => {
  await queue.add(randomUUID(), { fromEventTimestamp, toEventTimestamp });
};
