...
import { OrderKind } from "@reservoir0x/sdk/dist/seaport-v1.3/types";
...
import { offChainCheck, offChainCheckPartial } from "@/orderbook/orders/seaport-v1.3/check";
...
export type OrderInfo =
  | {
      kind: "full";
      orderParams: Sdk.SeaportV13.Types.OrderComponents;
...
export const save = async (
  ...
  const handleOrder = async (
    ...
    try {
      const order = new Sdk.SeaportV13.Order(config.chainId, orderParams);
      ...
      if (
        ![
          ...
          AddressZero,
          // Pausable zone
          Sdk.SeaportV13.Addresses.PausableZone[config.chainId],
      ...
      orderValues.push({
        ...
        conduit: toBuffer(
          new Sdk.SeaportV13.Exchange(config.chainId).deriveConduit(order.params.conduitKey)
        ),
        fee_bps: feeBps,
      ...
      if (totalTimeElapsed > 1) {
        logger.info(
          "orders-seaport-v1.3-save-debug-latency",
    ...
    try {
      ...
      if (orderParams.side === "buy" && orderParams.kind === "single-token" && validateBidValue) {
        ...
        try {
          ...
          logger.warn(
            "orders-seaport-v1.3-save-partial",
            `Bid value validation - error. orderId=${id}, contract=${orderParams.contract}, tokenId=${tokenId}, error=${error}`
          );
        }
      }

      const nonce = await commonHelpers.getMinNonce("seaport-v1.3", orderParams.offerer);
      ...
      orderValues.push({
        ...
        conduit: toBuffer(new Sdk.SeaportV13.Exchange(config.chainId).deriveConduit(conduitKey)),
        fee_bps: feeBps,
        fee_breakdown: feeBreakdown || null,
      ...
      limit(async () =>
        orderInfo.kind == "partial"
          ...
          : tracer.trace("handleOrder", { resource: "seaportV12Save" }, () =>
              handleOrder(
                orderInfo.orderParams as Sdk.SeaportV13.Types.OrderComponents,
                orderInfo.metadata,
