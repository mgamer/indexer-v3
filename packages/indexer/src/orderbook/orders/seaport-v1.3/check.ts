...
import { PartialOrderComponents } from "@/orderbook/orders/seaport-v1.3/index";
...
export const offChainCheck = async (
  ...
  const minNonce = await commonHelpers.getMinNonce("seaport-v1.3", order.params.offerer);

  if (!minNonce.eq(order.params.counter)) {
    throw new Error("cancelled");
  }

  const conduit = new Sdk.SeaportV13.Exchange(config.chainId).deriveConduit(
...
export const offChainCheckPartial = async (
  ...
  const conduit = new Sdk.SeaportV13.Exchange(config.chainId).deriveConduit(conduitKey);
