...
import { BaseBuildParams } from "@reservoir0x/sdk/dist/seaport-v1.3/builders/base";
...
export interface BaseOrderBuildOptions {
  ...
  orderType?: Sdk.SeaportV13.Types.OrderType;
  currency?: string;
  quantity?: number;
  nonce?: string;
  fee?: number[];
  ...
  const buildParams: BaseBuildParams = {
    ...
    zone:
      options.orderbook === "opensea"
        ? Sdk.SeaportV13.Addresses.PausableZone[config.chainId] ?? AddressZero
        : AddressZero,
    // Use OpenSea's conduit for sharing approvals (where available)
    conduitKey: Sdk.SeaportV13.Addresses.OpenseaConduitKey[config.chainId] ?? HashZero,
