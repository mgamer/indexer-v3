...
import { BaseBuilder } from "@reservoir0x/sdk/dist/seaport-v1.3/builders/base";

import { idb } from "@/common/db";
import { redis } from "@/common/redis";
import { fromBuffer } from "@/common/utils";
import { config } from "@/config/index";
import * as utils from "@/orderbook/orders/seaport-v1.3/build/utils";
...
export const build = async (options: BuildOrderOptions) => {
  ...
  if (collectionIsContractWide && !options.excludeFlaggedTokens) {
    ...
    let builder: BaseBuilder = new Sdk.SeaportV13.Builders.ContractWide(config.chainId);
    ...
    if (options.orderbook === "opensea" && config.chainId !== 5) {
      ...
      if (
        ...
        builder = new Sdk.SeaportV13.Builders.TokenList(config.chainId);
      }
    }
    ...
    const builder: BaseBuilder = new Sdk.SeaportV13.Builders.TokenList(config.chainId);
