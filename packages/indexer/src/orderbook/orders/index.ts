...
export * as seaportV13 from "@/orderbook/orders/seaport-v1.3";
    ...
    case "seaport-v1.3": {
      if (order.rawData && !order.rawData.partial) {
        return {
          kind: "seaport-v1.3",
          ...common,
          order: new Sdk.SeaportV13.Order(config.chainId, order.rawData),
        };
      } else {
        // Sorry for all the below `any` types
        return {
          // eslint-disable-next-line
          kind: "seaport-v1.3-partial" as any,
          ...common,
          order: {
            contract: token.contract,
            tokenId: token.tokenId,
  ...
  switch (order.kind) {
    ...
    case "seaport-v1.3": {
      if (order.rawData && !order.rawData.partial) {
        ...
        const sdkOrder = new Sdk.SeaportV13.Order(config.chainId, order.rawData);
        if (sdkOrder.params.kind?.includes("token-list")) {
          // When filling a "token-list" order, we also need to pass in the
          // full list of tokens the order was made on (in order to be able
          // to generate a valid merkle proof)
