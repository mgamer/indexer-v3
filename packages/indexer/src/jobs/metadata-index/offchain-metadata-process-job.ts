...
export type OffchainMetadataIndexProcessJobPayload = {
  method: string;
};

export class OffchainMetadataIndexProcessJob extends AbstractRabbitMqJobHandler {
  ...
  protected async process(payload: OffchainMetadataIndexProcessJobPayload) {
  ...
  public async addToQueue(params: OffchainMetadataIndexProcessJobPayload, delay = 0) {
    await this.send({ payload: params, jobId: params.method }, delay);
  }
}

export const offchainMetadataIndexProcessJob = new OffchainMetadataIndexProcessJob();
