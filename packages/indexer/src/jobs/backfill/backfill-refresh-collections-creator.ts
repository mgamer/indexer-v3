...
const QUEUE_NAME = "backfill-refresh-collections-creator";
...
if (config.doBackgroundWork) {
  const worker = new Worker(
    ...
    async (job: Job) => {
      ...
      if (cursor) {
        continuationFilter = `AND (collections.all_time_volume) < ($/allTimeVolume/)`;
      }

      const results = await idb.manyOrNone(
        `
          SELECT collections.id, collections.all_time_volume
          FROM collections
          WHERE collections.creator IS NULL
          ${continuationFilter}
          ORDER BY all_time_volume DESC
          LIMIT $/limit/
        `,
        {
          allTimeVolume: cursor?.allTimeVolume,
          limit,
        }
      );
      ...
      if (results.length == limit) {
        ...
        nextCursor = {
          allTimeVolume: lastResult.all_time_volume,
        };
