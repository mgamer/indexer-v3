...
// Seaport v1.3
import * as seaportV13BuyAttribute from "@/orderbook/orders/seaport-v1.3/build/buy/attribute";
import * as seaportV13BuyToken from "@/orderbook/orders/seaport-v1.3/build/buy/token";
import * as seaportV13BuyCollection from "@/orderbook/orders/seaport-v1.3/build/buy/collection";
    ...
    try {
      ...
      const bulkOrders = {
        "seaport-v1.3": [] as {
          order: {
            kind: "seaport-v1.3";
            data: Sdk.SeaportV13.Types.OrderComponents;
          };
          tokenSetId?: string;
          attribute?: {
            collection: string;
          ...
          try {
            ...
            switch (params.orderKind) {
              ...
              case "seaport-v1.3": {
                ...
                let order: Sdk.SeaportV13.Order;
                if (token) {
                  const [contract, tokenId] = token.split(":");
                  order = await seaportV13BuyToken.build({
                    ...params,
                    orderbook: params.orderbook as "reservoir" | "opensea",
                    maker,
                    contract,
                  ...
                  order = await seaportV13BuyAttribute.build({
                    ...params,
                    orderbook: params.orderbook as "reservoir" | "opensea",
                    maker,
                    source,
                  ...
                  order = await seaportV13BuyCollection.build({
                    ...params,
                    orderbook: params.orderbook as "reservoir" | "opensea",
                    maker,
                    collection,
                ...
                const exchange = new Sdk.SeaportV13.Exchange(config.chainId);
                const conduit = exchange.deriveConduit(order.params.conduitKey);
      ...
      // Post any bulk orders together
      {
        ...
        const orders = bulkOrders["seaport-v1.3"];
        if (orders.length) {
          const { signatureData, proofs } = exchange.getBulkSignatureDataWithProofs(
            orders.map((o) => new Sdk.SeaportV13.Order(config.chainId, o.order.data))
          );
