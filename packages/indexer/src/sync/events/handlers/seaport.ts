...
export const handleEvents = async (events: EnhancedEvent[], onChainData: OnChainData) => {
  ...
  for (const { subKind, baseEventParams, log } of events) {
    ...
    switch (subKind) {
      ...
      case "seaport-v1.3-order-cancelled": {
        const parsedLog = eventData.abi.parseLog(log);
        const orderId = parsedLog.args["orderHash"].toLowerCase();

        const orderKind = subKind.startsWith("seaport-v1.3") ? "seaport-v1.3" : "seaport";
      ...
      case "seaport-v1.3-counter-incremented": {
        const parsedLog = eventData.abi.parseLog(log);
      ...
      case "seaport-v1.3-order-filled": {
        const parsedLog = eventData.abi.parseLog(log);
        const orderId = parsedLog.args["orderHash"].toLowerCase();
        const maker = parsedLog.args["offerer"].toLowerCase();
        ...
        const orderKind = subKind.startsWith("seaport-v1.3") ? "seaport-v1.3" : "seaport";
        const exchange =
          orderKind === "seaport-v1.3"
            ? new Sdk.SeaportV13.Exchange(config.chainId)
