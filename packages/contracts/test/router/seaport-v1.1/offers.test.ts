...
import { SeaportOffer, setupSeaportOffers } from "../helpers/seaport-v1.1";
import {
  bn,
  getChainId,
  getRandomBoolean,
...
describe("[ReservoirV6_0_1] SeaportV11 offers", () => {
  ...
  const testAcceptOffers = async (
    ...
    for (const offer of offers) {
      ...
      await offer.nft.contract
        .connect(carol)
        .transferFrom(carol.address, seaportModule.address, offer.nft.id);
    }
