...
export const setupListings = async (listings: Listing[]) => {
  ...
  for (const listing of listings) {
    ...
    if (listing.isCancelled) {
      const exchange = new Sdk.SeaportV13.Exchange(chainId);
...
export const setupOffers = async (offers: Offer[]) => {
  ...
  for (const offer of offers) {
    ...
    await weth.approve(buyer, Sdk.SeaportV13.Addresses.Exchange[chainId]);
...
export const setupERC20Approvals = async (approvals: ERC20Approval[]) => {
  ...
  for (const approval of approvals) {
    ...
    await erc20.approve(giver, Sdk.SeaportV13.Addresses.Exchange[chainId]);
