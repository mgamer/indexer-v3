...
import * as Helpers from "../helpers/seaport-v1.3";
import {
  bn,
  getChainId,
  getRandomBoolean,
...
describe("[ReservoirV6_0_0] SeaportV13 listings", () => {
  ...
  let erc721: Contract;
  let router: Contract;
  let seaportV13ApprovalOrderZone: Contract;
  let seaportV13Module: Contract;
  ...
  beforeEach(async () => {
    ...
    seaportV13ApprovalOrderZone = await ethers
      .getContractFactory("SeaportV13ApprovalOrderZone", deployer)
      .then((factory) => factory.deploy());
    seaportV13Module = await ethers
      .getContractFactory("SeaportV13Module", deployer)
  ...
  const getBalances = async (token: string) => {
    if (token === Sdk.Common.Addresses.Eth[chainId]) {
      return {
        ...
        seaportV13Module: await ethers.provider.getBalance(seaportV13Module.address),
      ...
      return {
        ...
        seaportV13Module: await contract.getBalance(seaportV13Module.address),
  ...
  const testAcceptListings = async (
    ...
    const executions: ExecutionInfo[] = [
              ...
              data: uniswapV3Module.interface.encodeFunctionData("ethToExactOutput", [
                {
                  ...
                  recipient: seaportV13Module.address,
                  amountOut: listings
      ...
      listingsCount > 1
        ? {
            module: seaportV13Module.address,
            data: seaportV13Module.interface.encodeFunctionData(
    ...
    expect(balancesAfter.seaportV13Module).to.eq(0);
  ...
  it("Fill USDC listing with ETH", async () => {
    ...
    const executions: ExecutionInfo[] = [
      ...
      {
        module: seaportV13Module.address,
        data: seaportV13Module.interface.encodeFunctionData("acceptERC20Listing", [
          {
            parameters: {
              ...listing.order!.params,
              totalOriginalConsiderationItems: listing.order!.params.consideration.length,
    ...
    expect(ethBalancesAfter.seaportV13Module).to.eq(0);
  ...
  it("Fill USDC listing approval-less", async () => {
    ...
    const approval: Helpers.ERC20Approval = {
      giver: bob,
      filler: seaportV13Module.address,
      paymentToken: listing.paymentToken!,
      amount: listing.price,
      zone: seaportV13ApprovalOrderZone.address,
    };
    ...
    const executions: ExecutionInfo[] = [
      ...
      {
        module: seaportV13Module.address,
        data: seaportV13Module.interface.encodeFunctionData("matchOrders", [
          [
            {
              parameters: {
