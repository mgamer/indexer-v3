...
export const setupZoraListings = async (listings: ZoraListing[]) => {
  ...
  for (const listing of listings) {
    ...
    await moduleManager.setApprovalForModule(seller, Sdk.Zora.Addresses.Exchange[chainId], true);

    // Approve the exchange contract
    await nft.contract.connect(seller).mint(nft.id);
    await nft.contract
      .connect(seller)
      .setApprovalForAll(Sdk.Zora.Addresses.Erc721TransferHelper[chainId], true);
