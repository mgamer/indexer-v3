...
export type SeaportERC20Approval = {
  ...
  amount: BigNumberish;
  zone?: string;
  orders?: Sdk.SeaportV11.Order[];
};

export const setupSeaportERC20Approvals = async (approvals: SeaportERC20Approval[]) => {
  const chainId = getChainId();
  ...
  for (const approval of approvals) {
    ...
    await erc20.approve(giver, Sdk.SeaportV11.Addresses.Exchange[chainId]);
