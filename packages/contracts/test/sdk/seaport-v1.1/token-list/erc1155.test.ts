...
import * as SeaportV11 from "@reservoir0x/sdk/src/seaport-v1.1";
...
describe("SeaportV11 - TokenList ERC1155", () => {
  ...
  it("Build and fill buy order with partial filling", async () => {
    ...
    await weth.approve(buyer, SeaportV11.Addresses.Exchange[chainId]);

    // Approve the exchange for the sellers
    await weth.approve(seller1, SeaportV11.Addresses.Exchange[chainId]);
    await weth.approve(seller2, SeaportV11.Addresses.Exchange[chainId]);
    ...
    await nft.approve(seller1, SeaportV11.Addresses.Exchange[chainId]);
    await nft.approve(seller2, SeaportV11.Addresses.Exchange[chainId]);

    const exchange = new SeaportV11.Exchange(chainId);
