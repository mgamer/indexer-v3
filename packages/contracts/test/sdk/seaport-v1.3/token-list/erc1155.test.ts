...
import * as SeaportV13 from "@reservoir0x/sdk/src/seaport-v1.3";
...
describe("SeaportV13 - TokenList ERC1155", () => {
  ...
  it("Build and fill buy order with partial filling", async () => {
    ...
    await weth.approve(buyer, SeaportV13.Addresses.Exchange[chainId]);

    // Approve the exchange for the sellers
    await weth.approve(seller1, SeaportV13.Addresses.Exchange[chainId]);
    await weth.approve(seller2, SeaportV13.Addresses.Exchange[chainId]);
    ...
    await nft.approve(seller1, SeaportV13.Addresses.Exchange[chainId]);
    await nft.approve(seller2, SeaportV13.Addresses.Exchange[chainId]);

    const exchange = new SeaportV13.Exchange(chainId);
    const builder = new SeaportV13.Builders.TokenList(chainId);
