...
describe("SeaportV13 - SingleToken Erc721", () => {
  ...
  it("Build and fill sell order", async () => {
    ...
    await nft.approve(seller, SeaportV13.Addresses.Exchange[chainId]);

    const exchange = new SeaportV13.Exchange(chainId);

    const builder = new SeaportV13.Builders.SingleToken(chainId);
  ...
  it("Build and fill ERC20 sell order", async () => {
    ...
    // Approve the exchange
    await erc20.connect(buyer).approve(SeaportV13.Addresses.Exchange[chainId], price);
