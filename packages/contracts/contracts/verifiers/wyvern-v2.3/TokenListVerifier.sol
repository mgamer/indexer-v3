...
contract TokenListVerifier {
  ...
  function verifyErc721(bytes memory callData) public pure {
    ...
    (bytes32 root, bytes32[] memory proof) = abi.decode(callData.drop(100), (bytes32, bytes32[]));

    require(verify(proof, root, keccak256(abi.encodePacked(tokenId))), "Invalid merkle proof");
  }

  function verifyErc1155(bytes memory callData) public pure {
    uint256 tokenId = abi.decode(callData.slice(68, 32), (uint256));

    (bytes32 root, bytes32[] memory proof) = abi.decode(callData.drop(196), (bytes32, bytes32[]));
