...
library BytesUtils {
  function drop(bytes memory array, uint256 start) public pure returns (bytes memory result) {
    ...
    assembly {
      ...
      case 0 {
        ...
        for {
          ...
          let cc := add(add(add(array, lengthMod), mul(0x20, iszero(lengthMod))), start)
        } lt(mc, end) {
          mc := add(mc, 0x20)
          cc := add(cc, 0x20)
        } {
