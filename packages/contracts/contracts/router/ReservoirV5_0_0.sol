...
contract ReservoirV5_0_0 is Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    address transferSelectorNFT = ILooksRare(looksRare).transferSelectorNFT();
    looksRareTransferManagerERC721 = ILooksRareTransferSelectorNFT(transferSelectorNFT)
      .TRANSFER_MANAGER_ERC721();
    looksRareTransferManagerERC1155 = ILooksRareTransferSelectorNFT(transferSelectorNFT)
    ...
    wyvernV23Proxy = IWyvernV23ProxyRegistry(proxyRegistry).proxies(address(this));

    // Approve the token transfer proxy
    IERC20(weth).approve(IWyvernV23(wyvernV23).tokenTransferProxy(), type(uint256).max);
  ...
  function singleERC721ListingFill(
    ...
    if (exchangeKind == ExchangeKind.SEAPORT) {
      ...
      target = looksRare;
      if (selector != ILooksRare.matchAskWithTakerBidUsingETHAndWETH.selector) {
        revert UnexpectedSelector();
      }
    ...
    if (exchangeKind != ExchangeKind.SEAPORT && exchangeKind != ExchangeKind.WYVERN_V23) {
  ...
  function singleERC721ListingFillWithPrecheck(
    ...
    if (expectedOwner != address(0) && IERC721(collection).ownerOf(tokenId) != expectedOwner) {
  ...
  function batchERC721ListingFill(
    ...
    for (uint256 i = 0; i < length; ) {
      IERC721(collections[i]).safeTransferFrom(address(this), receiver, tokenIds[i]);
  ...
  function singleERC721BidFill(
    ...
    bool isApproved = IERC721(collection).isApprovedForAll(address(this), operator);
    if (!isApproved) {
    ...
    uint256 balance = IERC20(weth).balanceOf(address(this)) - wethBalanceBefore;
    if (unwrapWeth) {
  ...
  function singleERC1155ListingFill(
    ...
    if (exchangeKind != ExchangeKind.SEAPORT && exchangeKind != ExchangeKind.WYVERN_V23) {
      ...
      // the NFT to the taker's wallet after the fill (since we cannot have
      // a recipient other than the taker)
      IERC1155(collection).safeTransferFrom(address(this), receiver, tokenId, amount, "");
    }
  ...
  function singleERC1155ListingFillWithPrecheck(
    ...
    if (
      expectedOwner != address(0) && IERC1155(collection).balanceOf(expectedOwner, tokenId) < amount
    ) {
      revert UnexpectedOwnerOrBalance();
    }
  ...
  function singleERC1155BidFill(
    ...
    // Approve the exchange to transfer the NFT out of the router
    bool isApproved = IERC1155(collection).isApprovedForAll(address(this), operator);
    if (!isApproved) {
